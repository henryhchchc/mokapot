        ...
        let instruction = match opcode {
            ...
            0xab => {
                ...
                for _ in 0..npairs {
                    ...
                    if match_offsets
                        .last()
                        .is_some_and(|(previous, _)| match_value <= *previous)
                    {
                        Err(ParseError::malform(
                            "Lookupswitch match values are not strictly increasing",
                        ))?;
                    }
...
mod tests {
    ...
    proptest! {
        ...
        fn switches_reject_nonzero_padding(
            ...
            if opcode == 0xaa {
                bytes.extend([0; 16]); // default, low, high, and one jump offset
            } else {
                bytes.extend([0; 8]); // default and zero pair count
            }
