...
use crate::{
    ...
    jvm::code::{ExceptionTableEntry, Instruction, ProgramCounter},
};
...
#[test]
fn represents_jsr_continuation_and_resolves_ret_candidate() {
    let instructions = [
    ...
    let cfg = build(&method).unwrap();
    let jsr = cfg.block_at_pc(0.into()).unwrap();
...
fn ret_continuations(cfg: &BytecodeCfg, pc: u16) -> &BTreeMap<ProgramCounter, StructuralBlockId> {
    let StructuralTerminator::Ret { continuations, .. } =
        &cfg.block_at_pc(pc.into()).unwrap().terminator
    else {
        panic!("expected ret at {pc}");
    };
    continuations
}

#[test]
fn shared_subroutine_ret_collects_every_call_continuation() {
    let instructions = [
...
#[test]
fn disjoint_subroutines_keep_their_ret_candidates_separate() {
    let instructions = [
        (0, Instruction::Jsr(20.into())),
        (3, Instruction::Jsr(30.into())),
        (6, Instruction::Return),
        (20, Instruction::Ret(0)),
        (30, Instruction::Ret(1)),
    ];
    let method = method(instructions, "()V", vec![]);
...
#[test]
fn outer_subroutine_traversal_follows_nested_call_continuation() {
    let instructions = [
        (0, Instruction::Jsr(10.into())),
        (3, Instruction::Return),
        (10, Instruction::Jsr(20.into())),
        (13, Instruction::Ret(0)),
        (20, Instruction::Ret(1)),
    ];
    let method = method(instructions, "()V", vec![]);
...
#[test]
fn unreachable_jsr_still_resolves_its_ret_candidate() {
    let instructions = [
        (0, Instruction::Return),
        (1, Instruction::Jsr(10.into())),
        (4, Instruction::Return),
        (10, Instruction::Ret(0)),
    ];
    let method = method(instructions, "()V", vec![]);
...
#[test]
fn subroutine_traversal_reaches_ret_through_exception_handler() {
    let string_type = "java/lang/String".parse().unwrap();
    let instructions = [
        (0, Instruction::Jsr(10.into())),
        (3, Instruction::Return),
        (10, Instruction::AStore0),
        (11, Instruction::AConstNull),
        (12, Instruction::CheckCast(string_type)),
        (13, Instruction::Ret(0)),
        (20, Instruction::AStore1),
        (21, Instruction::Ret(1)),
    ];
    let exception_table = vec![ExceptionTableEntry {
        covered_pc: 12.into()..13.into(),
        handler_pc: 20.into(),
        catch_type: None,
    }];
    let method = method(instructions, "()V", exception_table);
