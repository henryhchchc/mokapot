...
impl Executor<'_> {
    pub(super) fn try_lift_locals(
        ...
        use JVM::{
            ALoad, ALoad0, ALoad1, ALoad2, ALoad3, AStore, AStore0, AStore1, AStore2, AStore3,
            DLoad, DLoad0, DLoad1, DLoad2, DLoad3, DStore, DStore0, DStore1, DStore2, DStore3,
            FLoad, FLoad0, FLoad1, FLoad2, FLoad3, FStore, FStore0, FStore1, FStore2, FStore3,
            IInc, ILoad, ILoad0, ILoad1, ILoad2, ILoad3, IStore, IStore0, IStore1, IStore2,
            IStore3, LLoad, LLoad0, LLoad1, LLoad2, LLoad3, LStore, LStore0, LStore1, LStore2,
            LStore3, Wide,
        };

        let instruction = match jvm_instruction {
            ILoad(idx) | FLoad(idx) | ALoad(idx) => {
                load_local::<CATEGORY_1>(frame, u16::from(*idx))?
            }
            ...
            IInc(idx, constant) => {
                ...
                increment_local(frame, (*idx).into(), *constant, value)?
            }
            Wide(WideInstruction::IInc(idx, constant)) => {
                let value = self.definition_id_at(location)?;
                increment_local(frame, *idx, *constant, value)?
            }
            Wide(
